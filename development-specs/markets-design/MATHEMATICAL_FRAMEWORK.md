...
### Discretization Strategy (on-chain)

To run on-chain efficiently, we approximate the continuous functions using a fixed grid and fixed-point arithmetic.

- **Domain truncation:** choose finite interval [a, b] that covers the bulk of mass (e.g., μ ± 6σ for Gaussians). Values outside contribute negligible mass.
- **Grid:** partition [a, b] into N bins of width Δx = (b − a)/N, with bin centers xᵢ = a + (i + 0.5)Δx.
- **Representation:** store fᵢ ≈ f(xᵢ) as fixed-point integers (e.g., Q32.32). Keep μ, σ in fixed-point as well.
- **Discretized L² norm:**
  ```
  ‖f‖₂ ≈ √(Σᵢ fᵢ² · Δx)
  ```
- **Invariant check:** enforce Σᵢ fᵢ² · Δx ≤ k² with rounding toward the trader’s disadvantage (ceil when increasing, floor when decreasing) to keep solvency.
- **Collateral (discretized):**
  ```
  Collateral ≈ −minᵢ [gᵢ − fᵢ]
  ```
  Evaluate at grid points; for safety, also check neighbors or apply a small downward bias to the minimum.
- **Parameter updates:** when trading in (μ, σ) space, compute fᵢ from φ(xᵢ; μ, σ) using polynomial approximations, re-scale by λ, then re-check the invariant and σ ≥ σ_min.
- **Bin count guidance:** N ∈ [64, 256] for typical trades; use larger N only off-chain or in keepers; consider piecewise-constant segments for adaptive refinement.
