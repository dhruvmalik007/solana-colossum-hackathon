...
## Solana Trading Flow & Settlement

### Order Execution (Hybrid)
...
sequenceDiagram
  ...
  U->>RP: place_order(size, side, limit?)
  ...
  RP->>T: transfers (payer→vaults, vaults→counterparty, fees→FeeVaultPDA)
  RP-->>U: receipt {avg_price, fees, collateral_locked}
```

### Collateral Handling

- **Required collateral** (`MATHEMATICAL_FRAMEWORK.md`):
  - `Collateral = -min_x [g(x) - f(x)] × Position_Size`
- **Lifecycle:** lock on open → adjust on modify → release/claim at resolution.
- **Margin calls:** UI should warn and block trades if projected collateral < threshold.

### Claims on Resolution
...
sequenceDiagram
  ...
  participant T as SPL Token
  MF-->>U: Market status = Resolved
  U->>MF: claim_payout()
  MF->>AMM: compute entitlement
  AMM->>T: transfer from VaultBasePDA → user
  MF-->>U: claim receipt
```

### Error States (On-Chain)

- **Insufficient collateral**: reject with message, suggest reduced size.
- **Slippage exceeded**: CLOB best and AMM quote exceeded tolerance.
- **Expired limit order**: cancel and notify.
...
1. **Market Filter Sidebar** (Left)
   ...
   **Time to Resolution:**
   - its based on the specific condition:
      - Webhooks to the given contract if they fetch the specific onchain / offchain parameter achieving the results.
      - or if there is timestamp defined (24hrs, 1d , 1week, ....). 
...
3. **Market Grid/List View** (Main area)
   ...
   **Distribution Preview:**
   - Mini chart (100x60px) showing current consensus distribution as the gaussian curve provided in the [gaussian description](../markets-design/DISTRIBUTIONAL_MARKETS_PROGRAM_DESIGN.md).
   ...
   **Key Metrics:**
   ...
   - **Time to Resolution**: Countdown or "Resolved as given stauts" as defined in the header.
...
- In-app banner: "Market resolution in progress..."
...
The workflow diagram should be shown as follows:
![alt text](image.png).
...
- showcasing the statistics of the orderbook of the investors in the given market super-imposed at the bar graph (as described in the distribution markets: https://www.paradigm.xyz/2024/12/distribution-markets).
- Button: "Claim Rewards" (large, primary)

**If Loss:**
- Card: "Position closed at loss: -$X,XXX"
- Explanation: "unfortunately you lost around $X,XXX"
- Need to present the statistics of what was the return in the chart distribution based on the distributional prediction provided by the user.
   - for e.g if the user bets around 100$ for a market prediction that the returns of the X pool on the Y protocol , the response should just showcase the orderbook of the investors in the given market super-imposed at the bar graph
- Learning: "View your prediction vs. actual result"
- Button: "View Orderbook"
...
5. Update: 
     - Portfolio and wallet balance refresh.
     - also the [agent](../agentic-design-workflow/AI_AGENTIC_WORKFLOWS.md) might recommend dynamically using copilotkit in order on which portfolio they can invest.
...
**Post-Resolution Analytics:**
   ...
   **Market Autopsy:**
   ...
   - TODO: AI Agent giving some insights in terms of the timeline of investment and patterns . for this you will be needing the integration of copilotKit components
...
- real time update effect for the charts and stats during login




### Empty States
- Portfolio: "No positions yet. Browse markets to start your first trade"
...
- **Mobile**: Touch-friendly hit targets (minimum 44x44px), responsive layouts and adaptable to the multi
